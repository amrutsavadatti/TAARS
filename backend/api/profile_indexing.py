...
from backend.profile_indexing_service import answer_question, get_index_status, index_active_profile
...
async def index_profile(x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    ...
    async with get_db() as db:
        try:
            status, chunks = await index_active_profile(db, configured_owner_id())
